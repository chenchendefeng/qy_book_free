package com.only.tech.task.configuration;
...
public class ZipkinConfig {
    ...
    public SpanCollector spanCollector() {
        Config config = Config.builder().compressionEnabled(false).connectTimeout(5000)
                .flushInterval(1).readTimeout(6000).build();
        return HttpSpanCollector.create(host + ":9411", config, new EmptySpanCollectorMetricsHandler());
    }
    ...
    public Brave brave(SpanCollector spanCollector) {
        Builder builder = new Builder("service" + port);// 指定serviceName
