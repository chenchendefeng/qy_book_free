...
import com.wy.adbook.di.component.DaggerLoginComponent
import com.wy.adbook.di.module.LoginModule
import com.wy.adbook.ext.onClick
import com.wy.adbook.ext.showKeyboard
...
//                    }
    ...
    private fun initListener() {
        ...
        code_clear_btn.onClick { enter_code_et.setText("") }
        phone_clear_btn.onClick { login_phone_et.setText("") }
        phone_login_btn.onClick {
            mPresenter?.login(
                login_phone_et.text.toString().trim(),
                enter_code_et.text.toString().trim()
            )
        }
        get_verification_code_tv.onClick {
            mPresenter?.getVerificationCode("86", login_phone_et.text.toString().trim())
        }
        wx_login_tv.onClick { mPresenter?.requestWx() }
    }
