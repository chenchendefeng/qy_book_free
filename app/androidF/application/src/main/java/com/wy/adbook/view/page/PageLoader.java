...
public abstract class PageLoader {
     ...
     * 广告的间距
     */
    private static int SHOW_AD_INTERVEL = 7;

    private int totalCurrentPage = 0;
    ...
    private void initData() {
        ...
        mPageStyle = mSettingManager.getReadBgTheme();
        // 初始化参数
    ...
    public void setPageStyle(PageStyle pageStyle) {
        if (pageStyle != PageStyle.NIGHT) {
            ...
            mSettingManager.setReadBackground(pageStyle);
        }
    ...
    private void prepareBook() {
        ...
        //mCurPage.position = mBookRecord.getPagePos();
    }
    ...
    private void drawBackground(Bitmap bitmap, boolean isUpdate) {
        ...
        if (!isUpdate) {
            ...
            if (!mChapterList.isEmpty()) {
                ...
                if (mContext instanceof Activity) {
                    boolean notchScreen = NotchTools.getFullScreenTools().isNotchScreen(((Activity) mContext).getWindow());
                    if (notchScreen) {
                        tipTop += NotchTools.getFullScreenTools().getNotchHeight(((Activity) mContext).getWindow());
                    }
                }
                //根据状态不一样，数据不一样
        ...
        //底部的字显示的位置Y
        float y = mDisplayHeight - mTipPaint.getFontMetrics().bottom - tipMarginHeight;
        String time = StringUtils.dateConvert(System.currentTimeMillis(), Constant.getFORMAT_TIME());
    ...
    private void drawContent(Bitmap bitmap) {
        ...
        if (mStatus != STATUS_FINISH) {
            ...
            if (mContext instanceof Activity) {
                boolean notchScreen = NotchTools.getFullScreenTools().isNotchScreen(((Activity) mContext).getWindow());
                if (notchScreen) {
                    top += NotchTools.getFullScreenTools().getNotchHeight(((Activity) mContext).getWindow());
                }
            }
    ...
    void prepareDisplay(int w, int h) {
        ...
        if (mContext instanceof Activity) {
            boolean notchScreen = NotchTools.getFullScreenTools().isNotchScreen(((Activity) mContext).getWindow());
            if (notchScreen) {
                mVisibleHeight -= NotchTools.getFullScreenTools().getNotchHeight(((Activity) mContext).getWindow());
            }
        }
        // 重置 PageMode
        mPageView.setPageMode(mPageMode);
    ...
    private boolean mIsSkipAd = false;

    public void isSkipAd(boolean isSkipAd) {
        this.mIsSkipAd = isSkipAd;
    }

    boolean isAddCoverView = false;
...
//        BookRecordBean mBookRecord = this.mBookRecord;
//        if (!isAddCoverView
//                && chapterPos == 0
//                && (mBookRecord == null || (mBookRecord.getChapter() == 0 && mBookRecord.getPagePos() == 0))) {
//            isAddCoverView = true;
...
//        }
            ...
            while (showTitle || (paragraph = br.readLine()) != null) {
                ...
                while (paragraph.length() > 0) {
                    ...
                    if (rHeight <= 0) {
                        ...
                        page.title = chapter.getTitle();//StringUtils.convertCC(chapter.getTitle(), mContext);
    ...
    private void addAdPage(List<TxtPage> pages, String title, int titleLinesCount) {
        if (mIsSkipAd) return;
        if (totalCurrentPage != 0 && totalCurrentPage % SHOW_AD_INTERVEL == 0) {
