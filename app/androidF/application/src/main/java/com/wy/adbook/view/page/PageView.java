...
import com.wy.adbook.view.animation.*;
...
//                canvas.drawColor(Color.YELLOW,PorterDuff.Mode.CLEAR);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (!isMove) {
                    isMove = Math.abs(mStartX - event.getX()) > slop
                            || Math.abs(mStartY - event.getY()) > slop;
                }
                // 如果滑动了，则进行翻页。
                if (isMove) {
                    mPageAnim.onTouchEvent(event);
    ...
    private void pageCancel() {
        ...
        if (mPageLoader.mCurPage != null
                && mPageLoader.mCurPage.isCustomView) {
            addAdLayout();
        } else {
            //否则清除
            cleanAdView();
...
//                    ViewParent parent = mCoverPageView.getParent();
//                    if (parent != null && parent == this) {
//                        mCoverPageView.setVisibility(View.VISIBLE);
//                    } else {
