 ...
 */
package com.freebookqy.viewlib.book.readview.manager;

import android.content.Context;
import android.text.TextUtils;
import com.freebookqy.utilslib.Utils;
import com.freebookqy.viewlib.book.Constant;
import com.freebookqy.viewlib.book.bean.BookLists;
import com.freebookqy.viewlib.book.bean.BookMixAToc;
import com.freebookqy.viewlib.book.bean.ChapterRead;
import com.freebookqy.viewlib.book.utils.*;
...
public class CacheManager {
    ...
    public List<BookLists.BookListsBean> getCollectionList() {
        List<BookLists.BookListsBean> list = (ArrayList<BookLists.BookListsBean>) ACache.get(
                Utils.INSTANCE.getApp()).getAsObject(getCollectionKey());
        ...
        for (BookLists.BookListsBean bean : list) {
            if (bean != null) {
                if (TextUtils.equals(bean._id, bookListId)) {
                    list.remove(bean);
                    ACache.get(Utils.INSTANCE.getApp()).put(getCollectionKey(), (Serializable) list);
                    break;
                }
            }
        }
    ...
    public synchronized String getCacheSize() {
        ...
        try {
            ...
            if (FileUtils.isSdCardAvailable()) {
                String extCacheDir = Utils.INSTANCE.getApp().getExternalCacheDir().getPath();
    ...
    public synchronized void clearCache(boolean clearReadPos, boolean clearCollect) {
        try {
            // 删除内存缓存
            String cacheDir = Utils.INSTANCE.getApp().getCacheDir().getPath();
            ...
            // 清除其他缓存
            ACache.get(Utils.INSTANCE.getApp()).clear();
        } catch (Exception e) {
            LogUtils.e(e.toString());
        }
    }
