    ...
    <el-table :data="users" highlight-current-row v-loading="listLoading" style="width: 100%;" border :header-cell-style="{background:'#eee'}">
      <el-table-column prop="userName" label="用户信息" min-width="80" align="center">
        <template slot-scope="scope">
          <el-button size="small" type="text" @click="handleEdit(scope.$index, scope.row)">{{scope.row.userName}}</el-button>
        </template>
      </el-table-column>
      <el-table-column prop="orderNum" label="成功订单数" min-width="80" show-overflow-tooltip align="center">
      </el-table-column>
      <el-table-column prop="orderMoney" label="订单总金额" min-width="80" align="center">
      </el-table-column>
...
<script>
import { userFirstPayList } from "@/apis/api";
  ...
  methods: {
    ...
    getUsers() {
      ...
      let para = {
        ...
        payDate: this.filters.value1
      };
      console.log(para);
      userFirstPayList(para)
        .then(res => {
