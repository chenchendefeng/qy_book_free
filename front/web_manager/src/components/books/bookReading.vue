<template>
  <section class="reding">
    ...
    <el-table :data="users" highlight-current-row v-loading="listLoading" style="width: 100%;" border :header-cell-style="{background:'#eee'}">
      ...
      <el-table-column prop="categoryList" label="分类" min-width="80" align="center">
        <template slot-scope="scope">
          <p v-for="(item,index) of scope.row.categoryList" :key="index">{{item}}</p>
        </template>
      </el-table-column>
      <!-- <el-table-column prop="loginName" label="章节" min-width="80" align="center">
      </el-table-column> -->
      <el-table-column prop="readingTime" label="时长（分）" min-width="100" align="center">
        <template slot-scope="scope">
          {{(scope.row.readingTime/60).toFixed(2)}}
        </template>
      </el-table-column>
      <el-table-column prop="loginName" label="customerName" min-width="120" align="center">
        <template slot-scope="scope">
          <el-button size="small" type="text" @click="handleEdit(scope.$index, scope.row)">{{scope.row.customerName}}</el-button>
        </template>
      </el-table-column>
      <el-table-column prop="createTime" label="时间" min-width="120" align="center">
      </el-table-column>
    </el-table>
...
<script>
import { readingTimeMgrlist } from "@/apis/api";
...
export default {
  ...
  mounted() {
    ...
    if (this.$store.state.book == "second") {
      this.filters.uid = this.$store.state.bookuid;
    }
    this.getUsers();
  },
  computed: {
